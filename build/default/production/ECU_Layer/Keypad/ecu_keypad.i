...
# 13 "ECU_Layer/Keypad/../../MCAL_Layer/GPIO/hal_gpio.h"
...
static const uint8 keypad_btns[((uint8)4)][((uint8)4)]={
...
STD_ReturnType keypad_initialize(const Keypad_t *keypad){
    ...
    if(keypad != ((void*)0)){
        for(uint8 row_counter = 0x00 ; row_counter < ((uint8)4) ; row_counter++){
            gpio_pin_initialize(&(keypad->keypad_rows[row_counter]));
        }
        for(uint8 column_counter = 0x00 ; column_counter < ((uint8)4) ; column_counter++){
...
STD_ReturnType keypad_get_pressed_button(const Keypad_t *keypad,uint8 *button_pressed){
    ...
    STD_ReturnType Status = (STD_ReturnType)0x01;
    Logic_t logic = ((uint8)0x00);

    if(keypad != ((void*)0) && button_pressed != ((void*)0)){
# 92 "ECU_Layer/Keypad/ecu_keypad.c"
        for(uint8 row_number = 0x00 ; row_number < ((uint8)4) ; row_number++){
            for(uint8 l_counter = 0x00; l_counter < ((uint8)4) ; l_counter++){
                gpio_pin_write_logic(&(keypad->keypad_rows[l_counter]),LOW);
            }
            gpio_pin_write_logic(&(keypad->keypad_rows[row_number]),HIGH);

            for(uint8 column_number = 0x00 ; column_number < ((uint8)4) ; column_number++){
