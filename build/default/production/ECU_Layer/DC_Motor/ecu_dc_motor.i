...
STD_ReturnType dc_motor_initialize(const dc_motor_t *_motor_){
    ...
    if(_motor_!= ((void*)0)){
        status = gpio_pin_direction_initialize(&(_motor_->motor[0x00U]));
        status = gpio_pin_direction_initialize(&(_motor_->motor[0x01U]));
    }
    else{
...
STD_ReturnType dc_motor_forward(const dc_motor_t *_motor_){
    ...
    if(_motor_ != ((void*)0)){
        ...
        status = gpio_pin_write_logic(&(_motor_->motor[0x00U]),HIGH);
        status = gpio_pin_write_logic(&(_motor_->motor[0x00U]),LOW);
    }
    else{
        status = (STD_ReturnType)0x00;
    }
